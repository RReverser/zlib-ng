...
#  define DOWORDSWAP(word) (word)
#  define TABLE crc_braid_table
#  define DO1 c = crc_table[(c ^ *buf++) & 0xff] ^ (c >> 8)
#elif BYTE_ORDER == BIG_ENDIAN
#  define DOSWAP(crc) ZSWAP32(crc)
#  define DOWORDSWAP(word) ZWORDSWAP(word)
#  define TABLE crc_braid_big_table
#  define DO1 c = crc_table[(c >> 24) ^ *buf++] ^ (c << 8)
#else
...
Z_INTERNAL uint32_t crc32_braid(uint32_t crc, const unsigned char *buf, uint64_t len) {
    ...
    if (len >= N * W + W - 1) {
        ...
        while (--blks) {
            ...
            crc0 = TABLE[0][word0 & 0xff];
#if N > 1
            crc1 = TABLE[0][word1 & 0xff];
#if N > 2
            crc2 = TABLE[0][word2 & 0xff];
#if N > 3
            crc3 = TABLE[0][word3 & 0xff];
#if N > 4
            crc4 = TABLE[0][word4 & 0xff];
#if N > 5
            crc5 = TABLE[0][word5 & 0xff];
#endif
#endif
#endif
#endif
#endif
            for (k = 1; k < W; k++) {
                crc0 ^= TABLE[k][(word0 >> (k << 3)) & 0xff];
#if N > 1
                crc1 ^= TABLE[k][(word1 >> (k << 3)) & 0xff];
#if N > 2
                crc2 ^= TABLE[k][(word2 >> (k << 3)) & 0xff];
#if N > 3
                crc3 ^= TABLE[k][(word3 >> (k << 3)) & 0xff];
#if N > 4
                crc4 ^= TABLE[k][(word4 >> (k << 3)) & 0xff];
#if N > 5
                crc5 ^= TABLE[k][(word5 >> (k << 3)) & 0xff];
#endif
#endif
#endif
#endif
